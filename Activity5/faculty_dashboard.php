...
<head>
    ...
    <style>
        ...
        .modal-content { background-color: #fff; margin: 5% auto; padding: 20px; border-radius: 8px; width: 50%; max-width: 500px; max-height: 85vh; overflow-y: auto; position: relative; }
...
<body>
    ...
    <nav class="navbar">
        <div class="nav-container">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item"><a href="#issues" class="nav-link" data-section="issues">Student Issues</a></li>
            ...
            <div class="sessions-container" id="sessions-container">
                ...
                <?php else: ?>
                    <?php foreach ($sessions as $session): ?>
                        <div class="session-card" data-session-id="<?php echo $session['session_id']; ?>">
                            <div style="display: flex; justify-content: space-between; align-items: start;">
                                <div style="flex: 1;">
                                    ...
                                    <?php if (!empty($session['attendance_code'])): ?>
                                        <div class="attendance-code-box" style="background: #e8f5e9; padding: 10px; border-radius: 4px; margin: 10px 0;">
                                            <p style="margin: 0;"><strong>Attendance Code:</strong> 
                                                <span class="code-display" style="font-size: 1.3em; font-weight: bold; color: #2e7d32;"><?php echo $session['attendance_code']; ?></span>
                                                <button onclick="copyCode('<?php echo $session['attendance_code']; ?>')" class="btn-secondary" style="margin-left: 10px; padding: 5px 10px;">Copy</button>
                                            </p>
                                            <p style="margin: 5px 0 0 0; font-size: 0.9em; color: #666;">
                                                Expires: <?php echo date('M d, Y h:i A', strtotime($session['code_expires_at'])); ?>
                                            </p>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                <div style="display: flex; gap: 5px;">
                                    <button onclick="editSession(<?php echo $session['session_id']; ?>)" class="btn-secondary" style="padding: 5px 10px;">✏️</button>
                                    <button onclick="deleteSession(<?php echo $session['session_id']; ?>)" class="btn-secondary" style="padding: 5px 10px; background: #e74c3c;">🗑️</button>
                                </div>
                            </div>
                            
                            <div style="display: flex; gap: 10px; margin-top: 15px; flex-wrap: wrap;">
                                <?php if (empty($session['attendance_code'])): ?>
                                    <button class="btn-primary" onclick="generateCode(<?php echo $session['session_id']; ?>)">Generate Attendance Code</button>
                                <?php else: ?>
                                    <button class="btn-primary" onclick="generateCode(<?php echo $session['session_id']; ?>)">Regenerate Code</button>
                                <?php endif; ?>
    ...
    <script src="scripts/faculty_dashboard.js?v=2.0"></script>
    <script src="scripts/attendance.js?v=2.0"></script>
    <script src="scripts/enrollment_requests.js?v=2.0"></script>
    <script src="scripts/student_issues.js?v=2.0"></script>
</body>
</html>
